 ...
 * );
 */
class DatabaseHandler extends ChainResponsibility
{
	private static $handlerAlias;
    /**
    ...
    {
        if (parent::$_handlerAlias == 'Database') {

	        $context['newRecord'] = array_merge($context['newRecord'], [
	            'msg' => $message,
                'user_id' => $_SESSION['Auth']['User']['id'], 
                'date_created' => date('Y-m-d H:i'),
                'user_name' => $_SESSION['Auth']['User']['full_name']
            ]);
                	        
	        $model = TableRegistry::get('Histories');
	        $test = $model->save($model->newEntity($context['newRecord']));//debug($test);
        }
        else {
            if ($this->getNext()) {
...
/*
	    $model = TableRegistry::get('Histories');
	    $test = $model->find()->first();//debug($test);
*/
/*
